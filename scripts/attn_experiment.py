"""Attention experiment: self-attention over agents in the world model.

Two changes vs scripts/scale_experiment.py:
  1. World model is AttnWorldModel: per-period input (N_agents, in_dim) ->
     input_proj -> MultiheadAttention block over agents -> per-agent MLP
     -> next-period state. This lets the world model encode cross-agent
     interactions (e.g., young HH's debt depends on bank's lending
     capacity AND on prime HHs' demand for the same loans).
  2. Ground-truth vocabulary has the two previously-unreachable
     conjunctive thresholds relaxed (p75 instead of fixed-multiplier),
     so durables_firm_high_inv and firm_AND_indebted_AND_high_inventory
     now fire at ~2-4% prevalence.

Configs (all on the AttnWorldModel acts feed):
  jr_w256_ep200   : control matching scale_experiment baseline width
  jr_w1024_ep200  : the width-sweep winner

We drop the L1 config here because it collapsed at the deeper substrate
in scale_experiment; investigating that is a separate question.

Usage:
    python scripts/attn_experiment.py
"""
...
from econsae.sae.world_model import (
    AttnWorldModel, WMTrainConfig, build_attn_data,
    extract_attn_h1_activations, train_attn_world_model,
)
from econsae.simulator.ensemble import generate_ensemble


RUNS_DIR = os.path.join(REPO_ROOT, "runs")
EXP_DIR = os.path.join(RUNS_DIR, "attn_experiment")
...
def main(seed: int = 0, n_trajectories: int = 128, n_periods: int = 100,
         wm_epochs: int = 40, embed_dim: int = 64, n_heads: int = 4,
         h1_dim: int = 192, h2_dim: int = 128, n_attn_layers: int = 1):
    torch.manual_seed(seed)
    t_start = time.time()
    print("=" * 78)
    print(f"ATTN EXPERIMENT  (n_traj={n_trajectories}, n_periods={n_periods})")
    print("=" * 78)
    ...
    # ---- 2. GT feature matrix (with relaxed conjunctive thresholds) ----
    t0 = time.time()
    ...
    print("    conjunctive feature prevalence (all 8 should now fire):")
    for j, name in enumerate(fm.feature_vocab):
        if feature_tier(name) == "conjunctive":
            print(f"      {prev[j]:>6.2%}  {name}")

    # ---- 3. Train AttnWorldModel ----
    t0 = time.time()
    wm_data = build_attn_data(ens.trajectories, ens.shock_schedules)
    print(f"\n[3] Attn training periods in {time.time() - t0:.1f}s: "
          f"X={tuple(wm_data.X.shape)} -> Y={tuple(wm_data.Y.shape)}")
    wm = AttnWorldModel(embed_dim=embed_dim, n_heads=n_heads,
                        n_attn_layers=n_attn_layers,
                        h1_dim=h1_dim, h2_dim=h2_dim)
    n_params = sum(p.numel() for p in wm.parameters())
    print(f"    AttnWorldModel: embed={embed_dim} heads={n_heads} "
          f"attn_layers={n_attn_layers} h1={h1_dim} h2={h2_dim}  "
          f"params={n_params:,}")
    t0 = time.time()
    res = train_attn_world_model(wm, wm_data,
                                  WMTrainConfig(epochs=wm_epochs, batch_size=128),
                                  verbose=False)
    ...
    }, os.path.join(RUNS_DIR, "world_model_attn.pt"))

    # ---- 4. Extract h1 ----
    t0 = time.time()
    H1, idx = extract_attn_h1_activations(wm, ens.trajectories, ens.shock_schedules)
    assert idx == fm.sample_index, "activation index mismatch"
    print(f"\n[4] Attn h1 extraction in {time.time() - t0:.1f}s: "
          f"H1={H1.shape} sparsity={(H1 == 0).mean():.1%}")
    np.savez_compressed(
        os.path.join(RUNS_DIR, "world_model_attn_acts.npz"),
    ...
    for name, variant, n_feat, kw, epochs in SAE_CONFIGS:
        ...
        torch.save({
            "state_dict": sae.state_dict(), "kind": variant, "feed_name": "acts_attn",
        ...
        rep.run_id = name; rep.feed_name = "acts_attn"; rep.variant = variant
        ...
        for j, gname in enumerate(feed.feature_vocab):
            if feature_tier(gname) == "conjunctive":
                ...
                marker = " (prev<0.5%)" if prev[j] < 0.005 else ""
        ...
        row.update({
            ...
            "conj_prevalence": {
                feed.feature_vocab[j]: float(prev[j])
                for j in range(len(feed.feature_vocab))
                if feature_tier(feed.feature_vocab[j]) == "conjunctive"
            },
        })
    ...
    # ---- 6. Comparison table ----
    print("\n" + "=" * 100)
    print(f"ATTN EXPERIMENT SUMMARY  ({n_trajectories}x{n_periods} ensemble, "
          f"AttnWorldModel substrate)")
    print("=" * 100)
    ...
    out_path = os.path.join(RUNS_DIR, "attn_experiment_summary.json")
